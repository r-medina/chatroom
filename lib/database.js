...
var db = function() {
  ...
  return {
    addMessage: function(message, callback) {
      var sql = 'INSERT INTO `messages` (`roomID`, `nick`, `color`, `body`, `timestamp`) VALUES ($1, $2, $3, $4, $5)';

      conn
        .query(sql, [message.roomID, message.user.nick, message.user.color, message.body, message.timestamp])
        .on('close', function () {
          if (callback) {
            callback();
          }
